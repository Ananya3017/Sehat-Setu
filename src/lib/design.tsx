...
body{font-family:'Mukta',sans-serif;background:#F5F7FA;color:#1A2B4A;min-height:100vh;}
...
.card-lift:hover{transform:translateY(-4px);box-shadow:0 14px 36px rgba(26,61,124,.14)!important;}
...
export const ProgBar = ({value,max=100,color=C.primary,h=7}) => (
  <div style={{background:C.borderLt,borderRadius:8,height:h,overflow:"hidden"}}>
    <div style={{height:"100%",borderRadius:8,background:color,
...
export const Card = ({children,style={},className="",onClick}) => (
  <div onClick={onClick} className={`card-lift ${className}`} style={{
    ...
    borderRadius:12,boxShadow:"0 2px 8px rgba(26,43,74,.07)",
    overflow:"hidden",cursor:onClick?"pointer":"default",...style,
  }}>{children}</div>
);

export const PageWrap = ({children,style={}}) => (
  <main className="page-enter" style={{maxWidth:1200,margin:"0 auto",
...
export const SecHead = ({label,title,sub}) => (
  <div style={{marginBottom:26}}>
    {label&&<div style={{fontSize:11,fontWeight:700,letterSpacing:"0.14em",
      textTransform:"uppercase",color:C.accent,marginBottom:7}}>{label}</div>}
    <h2 style={{fontSize:23,fontWeight:800,color:C.primary,
      fontFamily:"'Noto Serif Display',serif",lineHeight:1.25}}>{title}</h2>
...
export const StatCard = ({icon,label,value,sub,color=C.primary,delay=0,trend}) => (
  <Card className="page-enter" style={{padding:22,borderTop:`4px solid ${color}`,
    animationDelay:`${delay}s`}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
      <div>
        <div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",
          letterSpacing:"0.09em",color:C.textMuted,marginBottom:9}}>{label}</div>
