...
const Emergency = ({ user }) => {
  ...
  return (
    <div>
      <div style={{background:"linear-gradient(135deg,#7B0000,#B71C1C,#D32F2F)",
            ...
            <Card style={{padding:24,borderTop:`5px solid ${C.red}`,marginBottom:14}}>
              ...
              <div style={{display:"flex",gap:14,alignItems:"center",marginBottom:18}}>
                <div style={{width:52,height:52,borderRadius:"50%",
                  ...
                  fontSize:24,fontWeight:800,color:C.red}}>{eu.name[0]}</div>
                <div>
                  <div style={{fontSize:18,fontWeight:800,color:C.text,
                    fontFamily:"'Noto Serif Display',serif"}}>{eu.name}</div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
                {[{l:"Blood Group",v:eu.blood,c:C.red,i:"🩸"},
                  ...
                  <div key={x.l} style={{padding:"14px",background:`${x.c}08`,
              ...
              <div style={{marginBottom:13}}>
                <div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",
                  letterSpacing:"0.08em",color:C.textMuted,marginBottom:7}}>⚠️ Allergies</div>
                {eu.allergies?.length?(
                  <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                    {eu.allergies.map(a=>(
                      <span key={a} style={{padding:"4px 12px",background:C.amberLt,
                        border:`1px solid ${C.amber}44`,color:C.amber,
                        borderRadius:5,fontSize:12,fontWeight:700}}>⚠️ {a}</span>
                    ))}
                  </div>
              ...
              <div style={{marginBottom:14}}>
                <div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",
                  letterSpacing:"0.08em",color:C.textMuted,marginBottom:7}}>🩺 Chronic Conditions</div>
                {eu.conditions?.length?(
                  <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                    {eu.conditions.map(c=>(
                      <span key={c} style={{padding:"4px 12px",background:`${C.primary}10`,
                        border:`1px solid ${C.primary}30`,color:C.primary,
                        borderRadius:5,fontSize:12,fontWeight:700}}>{c}</span>
                    ))}
                  </div>
                ):<span style={{color:C.green,fontSize:12,fontWeight:600}}>✓ No chronic conditions</span>}
              </div>
              {/* Emergency contact */}
              <div style={{padding:"14px 16px",background:C.redLt,
                border:`1px solid ${C.red}30`,borderRadius:9}}>
                <div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",
                  letterSpacing:"0.08em",color:C.red,marginBottom:7}}>📞 Emergency Contact</div>
                ...
                <Btn label={eu.emergency.phone} icon="📞" variant="danger"
          ...
          <div>
            <Card style={{padding:24,textAlign:"center",marginBottom:14,
              ...
              <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:13,
                fontWeight:700,color:C.primary,marginBottom:4}}>{eu.abha}</div>
            ...
            <Card style={{padding:20,marginBottom:14}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                <Btn label="📞 Call Emergency Contact" style={{justifyContent:"center",padding:"11px",fontSize:13}} variant="danger"/>
                <Btn label="🏥 Locate Nearest Hospital" style={{justifyContent:"center",padding:"11px",fontSize:13}}/>
                <Btn label="💊 Full Medical History"    style={{justifyContent:"center",padding:"11px",fontSize:13}} variant="outline"/>
                <Btn label="🚑 Dispatch Ambulance (108)" style={{justifyContent:"center",padding:"11px",fontSize:13,background:"#7F0000",borderColor:"#7F0000"}} variant="danger"/>
              </div>
            </Card>
            ...
            <Card style={{padding:20}}>
              ...
              {[
                ...
                <div key={l.who} style={{display:"flex",justifyContent:"space-between",
