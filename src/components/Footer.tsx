...
export const Footer = () => (
  <footer style={{
    ...
    <div style={{maxWidth:1200,margin:"0 auto",display:"grid",
      ...
      <div>
        ...
        <div style={{marginTop:16,display:"flex",gap:12}}>
          {["T","f","in","IG"].map(soc=>(
            <span key={soc} style={{width:28,height:28,borderRadius:"50%",
              background:"rgba(255,255,255,.1)",display:"flex",alignItems:"center",
              justifyContent:"center",fontSize:12,fontWeight:600}}>{soc}</span>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,fontSize:13}}>
          {["Create ABHA Account","Hospitals & Clinics","Emergency Services",
            "Latest Health Schemes","PM-JAY Registration"].map(l=>(
            <span key={l} className="foot-link" style={{cursor:"pointer",
              transition:"color .2s"}}>{l}</span>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{marginTop:18,padding:"8px 12px",background:"rgba(255,255,255,.05)",
    ...
    {/* Bottom Copyright */}
    <div style={{maxWidth:1200,margin:"40px auto 0",paddingTop:20,
